...
struct coordpair
   {
   /* u, v can represent x,y or lon, lat  as convenient*/
   double u,v;
   };
struct coordpair ref;
struct coordpair *stncoords;

/* routine xy2lat returns a list of coordpairs */
struct coordpair *xy2lat(struct coordpair ref,struct coordpair *pts,int numpts)
   {
   double d2r,lonfactor,latfactor;
   int j;
   d2r = acos(-1.0)/180.;
   lonfactor = d2r*cos(d2r*ref.v)*
           (6378.139*(1.0-sin(d2r*ref.v)*sin(d2r*ref.v)/298.247));
   latfactor = 111.32;
   for (j=0;j<numpts;j++)
      {
      pts[j].u = pts[j].u/lonfactor + ref.u;
      pts[j].v = pts[j].v/latfactor + ref.v;
      }
   return pts;
   }


static int test,j,jj;
float  sa,ca;
 ...
 ****************************************************/
   ...
   /*
   * Support 2018 GeoGateway output (mbh-style)
   */
   strcpy(tfile,"disloc_table.txt");
   if ((fm = fopen(tfile,"w")) == NULL)
	 {
	   fprintf(stderr,"     ERROR:  cannot open %s\n",tfile);
	   exit(1);
	 }
 ...
 ****************************************************************/
   ...
   if (!data.genparm)
      ...
      data.label = (char **) malloc(sizeof(char *)*data.xnum);
      sum = (struct sumout * ) calloc( data.xnum, sizeof(struct sumout));
      for (k=0; k < data.xnum; ++k)
         {
         data.label[k] = (char *) malloc(sizeof(char)*MAX_NAME);
	 fscanf(fd,"%f %f",&data.xo[k],&data.yo[k]);
         /* 
         *  Allow optional station name or comment at end of line
         */
         if (fgets(comment,MAX_LINE,fd) != NULL)
            {
            if(strlen(comment) < 2){
               strcpy(comment,".");
               }
            comment[strlen(comment)] = '\0';
            /* Strip off leading white 
            */
            sscanf(comment,"%s",&comment);
            }
         strncpy(data.label[k],comment,MAX_NAME);
	 }
      }
   else /* Grid case */
      {
      ...
      data.xo = (float *) malloc(sizeof(float)*data.xnum*data.ynum);
      data.yo = (float *) malloc(sizeof(float)*data.xnum*data.ynum);
      data.label = (char **) malloc(sizeof(char *)*data.xnum*data.ynum);
      /* These labels will be blank for the grid case, but must be alloc'd. */
      for(k=0;k<data.xnum*data.ynum; k++)
         {
         data.label[k] = (char *) malloc(sizeof(char)*MAX_NAME);
   ...
   stncoords = malloc(sizeof(struct coordpair)*data.xnum*data.ynum);
   if (!test)
      {
	fprintf(fo,"%d  %d  %f  %f\n",data.xnum,data.ynum,
		latorig,lonorig);
 ...
 *********************************************************************/
	 if (data.genparm)
	    {
            data.xo[j] = dumx + (float)l * data.xinc;
            data.yo[j] = dumy + (float)k * data.yinc;
...
/*  finite fault  */
      ...
      ++j; /* j is indicating the station or grid points */
      }
   }

   j = 0;
   /* XXX write results to output file */
   fprintf(fo,"  x      y       ux        uy        uz        ");
   fprintf(fo,"exx       exy       eyy\n");
   ref.u = lonorig;
   ref.v = latorig;
   for (jj = 0; jj < data.xnum*data.ynum;jj++)
      {
      stncoords[jj].u = data.xo[jj];
      stncoords[jj].v = data.yo[jj];
      }
   stncoords = xy2lat(ref,stncoords,data.xnum*data.ynum);
   for (k=0; k < data.ynum; ++k)
      for (l=0; l < data.xnum; ++l)
         {
	 fprintf(fo,"  % 6.5f  % 6.5f  % 10.3e  % 10.3e  % 10.3e  ",
                 data.xo[j],data.yo[j], sum[j].d1,sum[j].d2,sum[j].d3);
	 fprintf(fo,"% 10.3e  % 10.3e  % 10.3e\n",sum[j].str1,sum[j].str2,
		 sum[j].str3);
         /* 
         * Gateway (mbh) file: Site Lon Lat DeltaE DeltaN DeltaV sig sig sig
         */
         fprintf(fm,"%s%12f%12f%12f%12f%12f%12f%12f%12f\n",
            data.label[j],stncoords[j].u,stncoords[j].v,
            sum[j].d1,sum[j].d2,sum[j].d3,sigdum,sigdum,sigdum);
         ++j;
         }
   fclose(fo); fclose(fd);fclose(fm);
}
/*********************************************************************/
