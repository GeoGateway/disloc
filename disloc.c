   ...
   while (fscanf(fd,"%f %f %f",&data.lon,&data.lat,&data.strike) != EOF)
	 ...
	 if (data.type)
	    printf("case %d       finite fault\n",n);
         printf("-------------------------\n");
         printf("x =" FLF ";  y =" FLF ";  d =" FLF ";",data.x[j],data.y[j],data.d);
	 printf("  delta =" FLF "; lambda =" FLF "; mu =" FLF ";\n",
	           data.delta,data.lambda,data.mu);
	 printf("d1 =" FLF "; d2 =" FLF "; d3 =" FLF "; l =" FLF "; w =" FLF ";\n",
	           data.u1,data.u2,data.u3,data.l,data.w);
         }
      else if (j == 0)
         {
	   fprintf(fo,"% 7.3f % 8.3f %8.3f\n",data.lon,data.lat,
		   data.strike);
	   fprintf(fo,
		   "" FLF " " FLF " " FLF " " FLF " " FLF " " FLF " " FLF " " FLF " " FLF "\n",
...
/*  finite fault  */
   ...
   for (k=0; k < data.ynum; ++k)
	 ...
	 fprintf(fo,"" FLF "  " FLF "  % 10.3e  % 10.3e  % 10.3e  ",xpr,ypr,
